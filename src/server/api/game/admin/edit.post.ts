...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { _id, platform, category, name, short_name, description, images } = body
    if(!_id || !category || !platform || !name || !short_name || !description) throw 'Dữ liệu đầu vào không hợp lệ'
