  ...
  <UiContent title="Payment" sub="Quản lý giao dịch nạp tiền">
    <UiFlex class="mb-4">
      ...
      <UForm :state="page" @submit="getList" class="mr-1">
        <UiFlex>
          <UInput v-model="page.search.key" placeholder="Tìm kiếm..." icon="i-bx-search" class="mr-1" />
    ...
    <UCard :ui="{ body: { padding: 'p-0 sm:p-0' } }">
      ...
      >
        ...
        <template #game-data="{ row }">
          <UBadge variant="soft" color="gray" class="cursor-pointer" @click="viewGame(row.game._id)">
            {{ row.game.short_name }}
          </UBadge>
        </template>
    ...
    <UModal v-model="modal.payment">
      <ServicePaymentView :fetch-id="statePayment" />
    </UModal>
