  ...
  <div>
    <UCard :ui="{ 
      ...
      <template #header>
        <UiFlex>
          ...
          <UButton color="gray" icon="i-bx-x" square variant="ghost" :padded="false" @click="emits('close')"></UButton>
        </UiFlex>
      </template>
      ...
      <UTable v-model:sort="page.sort" :columns="columns" :rows="list">
        ...
        <template #game-data="{ row }">
          <UBadge variant="soft" color="gray">{{ row.game.short_name }}</UBadge>
        </template>
    ...
    <UModal v-model="modal.payment" prevent-close>
      <UCard>
        ...
        <ServicePaymentView :fetch-id="statePayment" class="p-4"/>
      </UCard>
    </UModal>
